...
(function (root, factory) {
  ...
  var Collapse = function (element, options) {
    ...
    this.options.duration =
      isIE && isIE < 10 ? 0 : options.duration || this.duration;
    var self = this;
    var getOuterHeight = function (el) {
      var s = el && (el.currentStyle || window.getComputedStyle(el)),
        ...
        btp = /px/.test(s.borderTopWidth)
          ? Math.round(s.borderTopWidth.replace("px", ""))
          : 0,
        mtp = /px/.test(s.marginTop)
          ? Math.round(s.marginTop.replace("px", ""))
          : 0,
        mbp = /px/.test(s.marginBottom)
          ? Math.round(s.marginBottom.replace("px", ""))
          : 0,
        mte = /em/.test(s.marginTop)
          ? Math.round(s.marginTop.replace("em", "") * parseInt(s.fontSize))
          : 0,
        mbe = /em/.test(s.marginBottom)
          ? Math.round(s.marginBottom.replace("em", "") * parseInt(s.fontSize))
          : 0;
      return (
        el.clientHeight +
        parseInt(btp) +
        ...
        parseInt(mte) +
        parseInt(mbe)
      ); //we need an accurate margin value
    };
  ...
  var Tab = function (element, options) {
    ...
    this.handle = function (e) {
      ...
      var nextContent = document.getElementById(
        next.getAttribute("href").replace("#", "")
      ...
      if (!/\bactive/.test(next.parentNode.className)) {
        ...
        if (self.dropdown) {
          if (
            !/\bdropdown-menu/.test(self.tab.parentNode.parentNode.className)
          ) {
            if (/\bactive/.test(self.dropdown.className))
    ...
    this.getActiveTab = function () {
      ...
      if (
        activeTabs.length === 1 &&
        !/\bdropdown/.test(activeTabs[0].className)
      ) {
        return activeTabs[0];
  ...
  for (var tb = 0, tbl = Tabs.length; tb < tbl; tb++) {
    var tab = Tabs[tb],
      options = {};
    options.duration =
      (tab.getAttribute("data-duration") &&
        tab.getAttribute("data-duration")) ||
      false;
    new Tab(tab, options);
  }
