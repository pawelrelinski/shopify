...
export class ProductsListComponent implements OnInit {
	...
	private cutOffFirstPartOfCurrentRoute(): void {
		...
		this.currentRoute = startWith
			? this.currentRoute.slice(lengthOfUrlPartToCutOff, lengthOfCurrentRoute)
			: '';
	}

	private getSeparatedUrlParts(): Map<string, string> {
		const urlParts = this.currentRoute.split('/');
		return new Map<string, string>().set('category', urlParts[1]).set('productType', urlParts[2]);
	}
